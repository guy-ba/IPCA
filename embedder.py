...
# Encoder
encoder_hid_dim = 625  # 256
encoder_num_layers = 1
encoder_bidir = True
encoder_dropout = 0.5  # 0.5 , between layers
...
class Embedder(nn.Module):
    def __init__(self, dataset, name, decoder):
        super().__init__()
        self.name = name
        ...
        self.encoder_mu = nn.Linear(encoder_GRU_out_dim, embedding_dim)
        self.encoder_logVar = nn.Linear(encoder_GRU_out_dim, embedding_dim)
    ...
    def forward_encoder(self, tuple_strings):
        ...
        list_strings_emb = [self.decoder.char_embedder(string) for string in tuple_strings]
    ...
    def forward_decoder(self, tuple_strings, embedding):
        ...
        h_0 = self.decoder.decoder_emb_to_hid(embedding)
        h_0 = h_0.unsqueeze(0).repeat(self.decoder.decoder_GRU.num_layers, 1, 1)
        ...
        strings_emb = self.decoder.char_embedder(strings)
        ...
        # apply GRU decoder
        GRU_output, _ = self.decoder.decoder_GRU(GRU_input, h_0)
        GRU_output, _ = nn.utils.rnn.pad_packed_sequence(GRU_output, batch_first=True)

        # strings reconstruction and reconstruction loss
        strings_recon_score = self.decoder.decoder_GRU_out_to_char_score(GRU_output)
    ...
    def decoder_test(self, max_len=100, embedding=None, temp=1.0):
        with torch.no_grad():
            ...
            if embedding is None:
                embedding = torch.randn(batch_size, self.encoder_mu.out_features, device=self.decoder.char_embedder.weight.device)
            else:
                # prepare hidden state for GRU
                embedding = embedding.to(self.device)
                embedding_for_GRU_input = embedding.unsqueeze(1)
                h = self.decoder.decoder_emb_to_hid(embedding)
                h = h.unsqueeze(0).repeat(self.decoder.decoder_GRU.num_layers, 1, 1)
                ...
                for i in range(1, max_len):
                    ...
                    next_char_emb = self.decoder.char_embedder(next_char).unsqueeze(1)
                    GRU_input = torch.cat([next_char_emb, embedding_for_GRU_input], dim=-1) #input = next char + embedding
                    GRU_output, h = self.decoder.decoder_GRU(GRU_input, h)
                    strings_recon_score = self.decoder.decoder_GRU_out_to_char_score(GRU_output.squeeze(1))
