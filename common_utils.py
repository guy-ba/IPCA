...
def input2output(args, input_batch, model_in, T, model_out, random_seed_list=None, max_out_len=90, recover_seed=True):
    ...
    for seed in random_seed_list:
        ...
        if hasattr(args, 'no_embedding') and args.no_embedding:
            input_batch_emb = torch.zeros_like(input_batch_emb)
...
# check if output molecule is novel (not in trainset)
def is_novel(output_mol_smiles, trainset):
    return output_mol_smiles not in trainset
...
def process_results_file(res_file_path, args, valid_res_file_path, trainset):
    ...
    if valid_res_def.empty:
        valid_res_def = pd.DataFrame(columns=['input', 'output', 'QED', 'DRD2', 'sim', 'novel'])
        valid_res_def = valid_res_def.append({'input':'invalid', 'output':'invalid', 'QED':0, 'DRD2':0, 'sim':0, 'novel':0}, ignore_index=True)
    else:
        # add property column for output molecules
        valid_res_def['QED'] = np.vectorize(property_calc)(valid_res_def['output'], 'QED')
        valid_res_def['DRD2'] = np.vectorize(property_calc)(valid_res_def['output'], 'DRD2')
        ...
        valid_res_def['novel'] = np.vectorize(is_novel)(valid_res_def['output'], trainset)
...
def valid_results_file_to_metrics(valid_res_file_path, args, num_source_mols):
    ...
    validity, diversity, novelty, property_DRD, property_QED, similarity, SR = [], [], [], [], [], [], []
    for retry_i in range(10):
        validity_mean, diversity_mean, novelty_mean, property_DRD_mean, property_QED_mean, similarity_mean, SR_mean = \
        ...
        property_DRD.append(property_DRD_mean)
        property_QED.append(property_QED_mean)
        similarity.append(similarity_mean)
        SR.append(SR_mean)

    validity_np, diversity_np, novelty_np, property_DRD_np, property_QED_np, similarity_np, SR_np = np.array(validity), \
        np.array(diversity), np.array(novelty), np.array(property_DRD), np.array(property_QED), np.array(similarity), np.array(SR)
    return validity_np.mean(), validity_np.std(), \
           diversity_np.mean(), diversity_np.std(), \
           novelty_np.mean(), novelty_np.std(), \
           property_DRD_np.mean(), property_DRD_np.std(), \
           property_QED_np.mean(), property_QED_np.std(), \
...
def get_metics_for_sample(valid_df, args, how_many_samples, num_source_mols, seed):
    ...
    # calculate *** SR ***
    SR = len(valid_df[(valid_df['sim'] > args.SR_similarity) & (valid_df['QED'] > args.SR_property_QED_val) & (valid_df['DRD2'] > args.SR_property_DRD2_val)]) / num_valid_mols

    return validity, diversity, valid_df['novel'].mean(), valid_df['DRD2'].mean(), valid_df['QED'].mean(), valid_df['sim'].mean(), SR