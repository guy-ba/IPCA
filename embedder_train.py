...
def validation(args, model_name, model, boundaries, epoch, fig=None, ax=None):
    ...
    # dataset loader
    valid_loader = get_dataloader(model, args, model.dataset.validset, batch_size=args.validation_batch_size,
                                  shuffle=False)
    ...
    if collate_fn is None:
        def collate_fn(train_data):
            ...
            tensors = [model.dataset.string2tensor(string, model.dataset.c2i, device=model.device) for string in
                       train_data]
            return tensors
    return DataLoader(data, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn, num_workers=0,
                      worker_init_fn=set_seed(args.seed))


def get_valid_dataloader(model, args, data, batch_size=batch_size, collate_fn=None, shuffle=True):
    if collate_fn is None:
        def collate_fn(train_data):
            train_data.sort(key=len, reverse=True)
            tensors = [model.dataset.string2tensor(string.split()[0], model.dataset.c2i, device=model.device) for string
                       in
                       train_data]
            tensors_source = [string.split()[1] for string in train_data]
            return tensors, tensors_source
    return DataLoader(data, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn, num_workers=0,
                      worker_init_fn=set_seed(args.seed))
